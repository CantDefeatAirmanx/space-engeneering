...
func (s *TestingSuite) TestGetPartNotFound() {
	partUUID := gofakeit.UUID()
	s.mockRepo.EXPECT().GetPart(s.ctx, partUUID).Return(nil, &repository_part.ErrPartNotFound{})
...
func (s *TestingSuite) TestGetPartInternalError() {
	partUUID := gofakeit.UUID()
	s.mockRepo.EXPECT().GetPart(s.ctx, partUUID).Return(nil, &repository_part.ErrPartInternal{})
